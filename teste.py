...
def inserir_dados():
    ...
    if data and produto and preço and quantidade:
        cursor.execute('''
...
def exibir_grafico():
    cursor.execute('SELECT preço, produto FROM vendas')
    dados = cursor.fetchall()
    
    if dados:
        
        preços = [dado[0] for dado in dados]
        produtos = [dado[1] for dado in dados]
       
        plt.figure(figsize=(10, 5))
        plt.barh(produtos,preços,  color='skyblue')
        plt.xlabel('Preco')
        plt.ylabel('produto')
